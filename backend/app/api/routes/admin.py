...
from app.auth.schemas import CreateKeyRequest, APIKeyResponse, APIKeyInfo
from app.dependencies import get_db_session

router = APIRouter(prefix="/api/v1/admin", tags=["admin"])

@router.post("/keys", response_model=APIKeyResponse)
async def create_key(request: CreateKeyRequest, db: AsyncSession = Depends(get_db_session)):
    store = KeyStore(db)
...
async def revoke_key(key_prefix: str, db: AsyncSession = Depends(get_db_session)):
    store = KeyStore(db)
